...
public class FT_FCA_FunctionDef {
	...
	void loop2_i(@NonNull FT_FnCallArgs.DoAssignCall aDoAssignCall, final @NonNull ProcTableEntry pte, final @NonNull VariableTableEntry vte, final int instructionIndex) {
		...
		if (fd.getParent() == aDoAssignCall.generatedFunction.getFD().getParent()) {
			...
			if (fd.getParent() instanceof NamespaceStatement) {
				...
				invocation = ni;
			} else if (fd.getParent() instanceof final @NonNull ClassStatement classStatement) {
				@Nullable ClassInvocation     ci          = _inj().new_ClassInvocation(classStatement, null, new ReadySupplier_1<>(_dt2));
				final @NonNull List<TypeName> genericPart = classStatement.getGenericPart();
		...
		aDoAssignCall.dc.forFunction(aDoAssignCall.dc.newFunctionInvocation(fd, pte, invocation), new ForFunction() {
			...
			public void typeDecided(@NonNull GenType aType) {
				...
				@NonNull TypeTableEntry tte = aDoAssignCall.generatedFunction.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, aDoAssignCall.dc.gt(aType), pte.__debug_expression, pte);
