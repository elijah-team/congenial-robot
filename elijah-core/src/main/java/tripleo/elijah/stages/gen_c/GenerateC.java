...
public class GenerateC extends _GenerateC_T
		implements CodeGenerator, GenerateFiles, ReactiveDimension, SPI_Loggable, SPI_ReactiveDimension {
	...
	private final GenerateResultProgressive generateResultProgressive = new GenerateResultProgressive();
	...
	public GenerateC(final @NonNull OutputFileFactoryParams aParams, final @NonNull GenerateResultEnv aFileGen) {
		// provided
		_fileGen = aFileGen;
	...
	public void generateCodeForConstructor(final GenerateResultEnv aFileGen, final EvaConstructor gf) {
		final WhyNotGarish_Constructor cc = a_lookup(gf);

		cc.resolveFileGenPromise(aFileGen);
	}

	@NonNull
	public String getRealTargetName(final @NonNull BaseEvaFunction gf, final @NonNull IdentIA target,
	...
	String getRealTargetName(final @NonNull BaseEvaFunction gf, final @NonNull IntegerIA target,
	...
	@NonNull
	List<String> getArgumentStrings(final @NonNull Supplier<IFixedList<InstructionArgument>> instructionSupplier) {
	...
	String getRealTargetName(final @NonNull IntegerIA target, final Generate_Code_For_Method.AOG aog) {
	...
	@Deprecated
	public String getRealTargetName(final @NonNull WhyNotGarish_BaseFunction aGf, final @NonNull IntegerIA aTarget,
	...
	public @NonNull List<String> getArgumentStrings(final @NonNull WhyNotGarish_BaseFunction aGf,
			final @NonNull Instruction aInstruction) {
		return getArgumentStrings(aGf.cheat(), aInstruction);
	}

	@NonNull
	List<String> getArgumentStrings(final @NonNull BaseEvaFunction gf, final @NonNull Instruction instruction) {
		final WhyNotGarish_Function yf = a_lookup(gf);
		return yf.getArgumentStrings(instruction).getLeft();
	}

	private void generateCodeForConstructor(@NonNull DeducedEvaConstructor aEvaConstructor,
			GenerateResult aGenerateResult, WorkList aWorkList, final @NonNull GenerateResultEnv aFileGen) {
	...
	private void postGenerateCodeForConstructor(final @NonNull DeducedEvaConstructor aEvaConstructor,
			final @NonNull WorkList wl, final @NonNull GenerateResultEnv aFileGen) {
		for (IdentTableEntry identTableEntry : ((EvaConstructor) aEvaConstructor.getCarrier()).idte_list) {
	...
	private void generateIdent(@NonNull IdentTableEntry identTableEntry) {
		...
		final @NonNull GenerateResultEnv fileGen = _fileGen;
	...
	@Override
	public void generate_constructor(final IPP_Constructor aGf, final GenerateResult gr, final WorkList wl,
	...
	@Override
	public void generate_function(IPP_Function ipf, GenerateResult gr, @NonNull WorkList wl,
	...
	@Override
	public void generate_namespace(final @NonNull EvaNamespace x, final GenerateResult gr,
	...
	public @NonNull String getAssignmentValue(final VariableTableEntry aSelf, final InstructionArgument aRhs,
			final @NonNull WhyNotGarish_BaseFunction aGf) {
		return getAssignmentValue(aSelf, aRhs, aGf.cheat());
	}

	@NonNull
	public String getAssignmentValue(VariableTableEntry value_of_this, final InstructionArgument value,
	...
	private void _post_generate_function(final @NonNull EvaFunction aEvaFunction, final @NonNull WorkList wl,
...
//    throw new UnintendedUseException();
	...
	enum GetTypeName {
		...
		static String forOSType(final @NonNull OS_Type ty, @NonNull ElLog LOG) {
			...
			case BUILT_IN -> {
				...
				z = "Z" + ty.getBType().getCode(); // README should not even be here, but look at .name() for other
													// code gen schemes
			}
	...
	static class WlGenerateFunctionC implements WorkJob {
		...
		public WlGenerateFunctionC(@NonNull GenerateResultEnv fileGen, BaseEvaFunction aGf,
		...
		public void run(WorkManager aWorkManager) {
			if (gf instanceof EvaFunction) {
				...
				final PP_Function ppFunction = new PP_Function(((GenerateC) generateC).deduced(evaFunction),
				...
				final PP_Constructor ppConstructor = new PP_Constructor(((GenerateC) generateC).deduced(evaConstructor),
						(DeducedBaseEvaFunction dgf) -> {
							// dgf.generateCodeForMethod(gcfm, aFileGen);
							throw new UnintendedUseException();
						});
	...
	public static class GetAssignmentValue {
		...
		public @NonNull Operation<EG_Statement> FnCallArgs(@NonNull FnCallArgs fca, @NonNull BaseEvaFunction gf,
				@NonNull ElLog LOG) {
		...
		public GetAssignmentValueArgsStatement getAssignmentValueArgs(final @NonNull Instruction inst,
				final @NonNull BaseEvaFunction gf, @NonNull ElLog LOG) {
		...
		// TODO look at me
		public String forClassInvocation(@NonNull Instruction aInstruction, ClassInvocation aClsinv,
				@NonNull BaseEvaFunction gf, @NonNull ElLog LOG) {
			...
			@NonNull
			List<String> x = getAssignmentValueArgs(aInstruction, gf, LOG).stringList();
			reference.args(x);
	...
	@Override
	public @NonNull GenerateResult generateCode(final @NonNull Collection<EvaNode> lgn,
			final @NonNull GenerateResultEnv aFileGen) {
	...
	private record __SPI_Save(OutputFileFactoryParams params, GenerateResultEnv fileGen,
	...
	public class GenerateResultProgressive {
		...
		public void addFunction_lagging(final DeducedBaseEvaFunction gf, final Buffer buf, final GenerateResult.TY aTY,
		...
		public void addFunction(final @NonNull DeducedBaseEvaFunction aGf, final Buffer aBufHdr,
				final GenerateResult.TY aTY) {
			final LibraryStatementPart lsp = aGf.getModule__().getLsp();

			_gr.addFunction(new PP_Function(aGf), aBufHdr, aTY, lsp);
		}

		public void addConstructor_lagging(final DeducedEvaConstructor gf, final Buffer buf,
				final GenerateResult.TY aTY, final GenerateResult gr) {
			final LibraryStatementPart lsp = gf.evaLayer_module_lsp();
		...
		public void addConstructor(final @NonNull DeducedEvaConstructor aGf, final Buffer aBuf,
				final GenerateResult.TY aTY) {
	...
	public void generateCodeForMethod(final GenerateResultEnv aFileGen, final IEvaFunctionBase aEvaFunction) {
	...
	@Override
	public GenerateResult resultsFromNodes(final @NonNull List<EvaNode> aNodes, final WorkManager wm,
			final @NonNull GenerateResultSink grs, @NonNull final GenerateResultEnv fg) {
		...
		for (final EvaNode generatedNode : aNodes) {
			if (generatedNode instanceof final @NonNull EvaContainerNC nc) {
				...
				final @NonNull Collection<EvaNode> gn1 = (nc.functionMap.values()).stream().map(x -> (EvaNode) x)
						.collect(Collectors.toList());
				final GenerateResult gr3 = this.generateCode(gn1, fg);
				grs.additional(gr3);

				final @NonNull Collection<EvaNode> gn2 = (nc.classMap.values()).stream().map(x -> (EvaNode) x)
						.collect(Collectors.toList());
