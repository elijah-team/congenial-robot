...
//				LOG.err("8885 " +ty.attached);
    ...
    void generateCodeForConstructor(final @NotNull DeducedEvaConstructor gf,
        ...
        final GenerateC generateC = this.gc;
        final WhyNotGarish_Constructor yf = gf.getWhyNotGarishFunction(generateC);
...
//		GenerateResult gr = cfm.getGenerateResult();
        ...
        final GCFC reactivable_gcfc = new GCFC(rs, gf, gr);
    ...
    void generateCodeForMethod(final @NotNull DeducedEvaFunctionBase gf, final @NotNull GenerateResultEnv aFileGen) {
        ...
        final GCFM reactivable_gcfm = new GCFM(rs, (DeducedBaseEvaFunction) gf, gr);

        gf.reactive().add(reactivable_gcfm);

        final GenerateC generateC = this.gc;

        if (!DebugFlags.GCFM_MANUAL_DISABLED) {
            reactivable_gcfm.respondTo(generateC);
        }
        ...
        if (sink != null) {
            sink.addFunction(new PP_Function((DeducedBaseEvaFunction) gf), rs, generateC);
        } else {
