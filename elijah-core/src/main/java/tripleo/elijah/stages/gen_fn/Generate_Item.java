...
//			throw new NotImplementedException();
	...
	public void generate_construct_statement(@NonNull ConstructStatement aConstructStatement, @NonNull BaseEvaFunction gf, @NonNull Context cctx) {
		...
		l.add(procIA);
		final @NonNull List<InstructionArgument> args1 = generateFunctions.simplify_args(args, gf, cctx);
		l.addAll(args1);
		final int instruction_number = generateFunctions.add_i(gf, InstructionName.CONSTRUCT, l, cctx);
	}

	void generate_if(@NonNull final IfConditional ifc, final @NonNull BaseEvaFunction gf) {
...
//						label_next = gf.addLabel();
	...
	void generate_loop(@NonNull final Loop loop, final @NonNull BaseEvaFunction gf) {
...
//			LOG.info("702 "+loop.getType());
	...
	private void generate_loop_FROM_TO_TYPE(@NonNull Loop loop, @NonNull BaseEvaFunction gf, @NonNull Context cctx) {
	...
	private void generate_loop_EXPR_TYPE(@NonNull Loop loop, @NonNull BaseEvaFunction gf, @NonNull Context cctx) {
...
//				add_i(gf, InstructionName.AGN, List_of(new IntegerIA(loop_iterator), ia1), cctx);
	...
	void generate_match_conditional(@NonNull final MatchConditional mc, final @NonNull BaseEvaFunction gf) {
		final int     y    = 2;
...
//				LOG.info("710 " + i);
...

			{
				for (final MatchConditional.MC1 part : mc.getParts()) {
					if (part instanceof final MatchConditionalImpl.@NonNull MatchArm_TypeMatch mc1) {
						...
						gf.place(label_next);
						label_next = gf.addLabel();
					} else if (part instanceof final MatchConditionalImpl.@NonNull MatchConditionalPart2 mc2) {
...
//					break;
	...
	void generate_variable_sequence(@NonNull VariableSequence item, @NonNull BaseEvaFunction gf, @NonNull Context cctx) {
