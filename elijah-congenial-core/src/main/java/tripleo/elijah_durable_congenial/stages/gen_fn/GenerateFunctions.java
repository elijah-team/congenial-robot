...
import tripleo.elijah_durable_congenial.lang.impl.NumericExpressionImpl;
...
import tripleo.elijah_durable_congenial.stages.gdm.GDM_DotExpression;
import tripleo.elijah_durable_congenial.stages.gdm.GDM_IdentExpression;
import tripleo.elijah_durable_congenial.stages.gdm.GDM_Item;
...
*/
	...
	//	module = aModule;
	//	//
	//	LOG = new ElLog(module.getFileName(), _phase().getVerbosity(), PHASE);
	//
	//	bc.set(pa0);
	//	bc.addLog(LOG);
	//}
...
//			LOG.info("Skip namespace for now "+((NamespaceStatement) item).name());
	...
	void simplify_procedure_call(@NotNull final ProcedureCallExpression pce, final @NotNull BaseEvaFunction gf, final @NotNull Context cctx) {
		...
		{
			@NotNull ProcTableEntry pte = getProcTableEntry_(gf, i);
			if (expression_num instanceof IdentIA) {
				@NotNull IdentTableEntry idte = ((IdentIA) expression_num).getEntry();
				idte.setCallablePTE(pte);
				pte.typePromise(new DoneCallback<GenType>() { // TODO should this be done here?
					@Override
	...
	private @NotNull ProcTableEntry getProcTableEntry_(final BaseEvaFunction aGf, final int aI) {
		var pte = aGf.getProcTableEntry(aI);
...
//			LOG.err(String.format("108 %s %s", arg, type));
	...
	@NotNull
	public EvaFunction generateFunction(@NotNull final FunctionDef fd,
...
//				LOG.err("109 "+expression);
	...
	public InstructionArgument simplify_expression(@NotNull final IExpression expression, final @NotNull BaseEvaFunction gf, final @NotNull Context cctx) {
...
//						throw new IllegalStateException("Cant be here");
	...
	public void generateFromEntryPoints(final @NotNull GN_PL_Run2.GenerateFunctionsRequest rq) {
		...
		// FIXME looking too hard into _phase()...
		final WorkManager wm = _phase().getWm();
		...
		if (!wl.isEmpty()) {
			...
			c.reports().doOutputOn(Finally.Outs.Out_1069, //"1096"
								   "** 1070 WorkList not empty w/ " + wl.getJobs().size() + " items");
