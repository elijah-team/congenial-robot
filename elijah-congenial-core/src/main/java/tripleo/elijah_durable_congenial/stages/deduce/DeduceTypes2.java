...
import tripleo.elijah_congenial_durable.advanced.Times;
import tripleo.elijah_congenial_durable.deduce2.DerivedClassInvocation;
...
import tripleo.elijah_durable_congenial.lang.nextgen.names.impl.*;
...
import tripleo.elijah_durable_congenial.lang2.SpecialFunctions;
import tripleo.elijah_durable_congenial.lang2.SpecialVariables;
...
import tripleo.elijah_durable_congenial.stages.deduce.nextgen.*;
import tripleo.elijah_durable_congenial.stages.deduce.post_bytecode.*;
import tripleo.elijah_durable_congenial.stages.deduce.tastic.*;
import tripleo.elijah_durable_congenial.stages.gdm.GDM_IdentExpression;
import tripleo.elijah_durable_congenial.stages.gdm.GDM_VariableTableEntry;
...
public class DeduceTypes2 {
	...
	private final          DeducePhase              phase1;
	...
	private final          ErrSink                  errSink1;
	...
	public DeduceTypes2(DeduceTypes2Request aRequest) {
		...
		DebugCheckpoint.checkPoint("************* DT2::for " + module.getFileName());

		this.phase1   = rosetta.getDeducePhase();
		this.errSink1 = rosetta.getErrSink();
	...
	private IInvocation _deferred_member_invocation(final @NotNull DeduceElementWrapper aParent, final @Nullable IInvocation aInvocation) {
		...
		if (aInvocation == null) {
			if (aParent.isNamespaceStatement()) {
				invocation = _phase().registerNamespaceInvocation((NamespaceStatement) aParent.element());
	...
	public void deduceOneClass(final @NotNull EvaClass aEvaClass) {
		for (EvaContainer.VarTableEntry entry : aEvaClass.varTable) {
			...
			GenType       genType = GenType.makeFromOSType(vt, aEvaClass.ci.genericPart(), this, _phase(), LOG, errSink1);
			if (genType != null) {
	...
	public void deduceFunctions(final @NotNull Iterable<EvaNode> lgf) {
		for (final EvaNode evaNode : lgf) {
			if (evaNode instanceof @NotNull final EvaFunction generatedFunction) {
				deduceOneFunction(generatedFunction, _phase());
			}
		}
		@NotNull List<EvaNode> generatedClasses = (_phase().generatedClasses.copy());
		// TODO consider using reactive here
		int size;
		do {
			size             = df_helper(generatedClasses, new dfhi_functions());
			generatedClasses = _phase().generatedClasses.copy();
		} while (size > 0);
		do {
				...
				if (a != null) {
					...
					case USER:
						...
						try {
							...
							_errSink().reportDiagnostic(aResolveError);
						}
						break;
					default:
						int y3 = 2;
...
//				aResolveError.printStackTrace();
		...
		if (genType.getCi() == null) {
			genType.genCI(genType.getNonGenericTypeName(), this, _errSink(), _phase());
		}
		if (genType.getNode() == null) {
			if (genType.getCi() instanceof ClassInvocation) {
				WlGenerateClass gen = _inj().new_WlGenerateClass(getGenerateFunctions(module), (ClassInvocation) genType.getCi(), _phase().generatedClasses, _phase().codeRegistrar);
				gen.run(null);
				gen.resultPromise(genType::setNode);
			} else if (genType.getCi() instanceof NamespaceInvocation) {
				WlGenerateNamespace gen = _inj().new_WlGenerateNamespace(getGenerateFunctions(module), (NamespaceInvocation) genType.getCi(), _phase().generatedClasses, _phase().codeRegistrar);
				gen.run(null);
	...
	public @NotNull GenerateFunctions getGenerateFunctions(@NotNull OS_Module aModule) {
		return _phase().generatePhase.getGenerateFunctions(aModule);
	}
...
//				final InstructionArgument constTableIA = (instruction.getArg(1));
		{
			...
			resolveIdentIA_(context, identIA, generatedFunction, new FoundElement(_phase()) {
...
//							identIA.getEntry().setCallablePTE(pte); // TODO ??
					...
					if (fd instanceof DefFunctionDef) {
						...
						forFunction(newFunctionInvocation((FunctionDef) e, pte, invocation, _phase()), new ForFunction() {
							@Override
				...
				public void noFoundElement() {
					_errSink().reportError("370 Can't find callsite " + x);
	...
	public void resolveIdentIA_(@NotNull Context context, @NotNull IdentIA identIA, @NotNull BaseEvaFunction generatedFunction, @NotNull FoundElement foundElement) {
		@NotNull Resolve_Ident_IA ria = _inj().new_Resolve_Ident_IA(_inj().new_DeduceClient3(this), context, identIA, generatedFunction, foundElement, _errSink());
		ria.action();
	}
	...
	public void forFunction(@NotNull FunctionInvocation gf, @NotNull ForFunction forFunction) {
		_phase().forFunction(this, gf, forFunction);
	}
...
//		vte.type = cte.type;
	...
	public void onEnterFunction(final @NotNull BaseEvaFunction generatedFunction, final Context aContext) {
		...
		for (ProcTableEntry procTableEntry : generatedFunction.prte_list) {
			procTableEntry.setDeduceTypes2(this, aContext, generatedFunction, _errSink());
		}
		//
...
//                                              LOG.info("704 "+vte.type.attached+" "+vte.potentialTypes());
		...
		}
		{
			...
			final Resolve_each_typename ret = _inj().new_Resolve_each_typename(_phase(), this, _errSink());
...
//                                              }
			...
			_phase().addDrs(generatedFunction, generatedFunction.drs);

			_phase().doneWait(this.rosetta);
		}
		...
		for (DT_External external : externals) {
			//external.onTargetModule(tm -> {_phase().modulePromise(tm, external::actualise);}); //[T1160118]
			_phase().modulePromise(external.targetModule(), external::actualise);
		}
	}
	...
	void resolve_function_return_type(@NotNull BaseEvaFunction generatedFunction) {
		...
		final GenType gt = f.resolve_function_return_type_int(_errSink());
		if (gt != null)
			//_phase().typeDecided((EvaFunction) generatedFunction, gt);
...
//								variableTableEntry.dlv.type.resolve(yy);
		...
		if (vte.getVtt() == VariableTableType.ARG) {
			...
			if (attached != null) {
				if (attached.getType() == OS_Type.Type.USER) {
					//throw new AssertionError();
					_errSink().reportError("369 ARG USER type (not deduced) " + vte);
				}
			} else {
	...
	@NotNull DeferredMemberFunction deferred_member_function(OS_Element aParent,
		...
		IInvocation                           invocation = aInvocation;

		assert invocation == aFunctionInvocation;

		@NotNull DeferredMemberFunction dm = null;

		if (invocation == null) {
			if (aParent instanceof NamespaceStatement) {
				invocation = _phase().registerNamespaceInvocation((NamespaceStatement) aParent);
				dm           = _inj().new_DeferredMemberFunction(aParent, invocation, aFunctionDef, this, aFunctionInvocation);
				_phase().addDeferredMember(dm);
			}
			else if (aParent instanceof OS_SpecialVariable) {
				dm = _inj().new_DeferredMemberFunction(aParent, invocation, aFunctionDef, this, aFunctionInvocation);
				final @NotNull DeferredMemberFunction finalDm = dm;
				((OS_SpecialVariable) aParent).onInvocation(this, new DoneCallback<IInvocation>() {
					@Override
					public void onDone(final IInvocation result) {
						_phase().addDeferredMember(finalDm);
					}
				});
			}
		}
	...
	public DG_ClassStatement DG_ClassStatement(final ClassStatement aClassStatement) {
		...
		final DG_ClassStatement R = _inj().new_DG_ClassStatement(aClassStatement, this);
	...
	public void register_and_resolve(@NotNull VariableTableEntry aVte, @NotNull ClassStatement aKlass) {
		...
		ci = _phase().registerClassInvocation(ci);
		ci.resolvePromise().then(aVte::resolveTypeToClass);
	}
	...
	public static class DeduceClient2 {
		...
		//	return deduceTypes2._phase().registerClassInvocation(new RegisterClassInvocation_env(ci, deduceTypes2, deduceTypes2.phase));
		//}
	...
	public static class DeduceClient1 {
		...
		public void genCI(final @NotNull GenType aResult, final TypeName aNonGenericTypeName) {
			aResult.genCI(aNonGenericTypeName, dt2, dt2._errSink(), dt2._phase());
		}
		...
		public @Nullable ClassInvocation registerClassInvocation(final @NotNull ClassStatement aClassStatement, final String aS) {
			return dt2._phase().registerClassInvocation(aClassStatement, aS, new ReadySupplier_1<>(dt2));
		}
...
*/
	...
	public static class OS_SpecialVariable implements OS_Element {
		...
		private Eventual<IInvocation> _p_Invocation;
		...
		public void onInvocation(final @NotNull DeduceTypes2 aDeduceTypes2, final DoneCallback<IInvocation> cb) {
			if (_p_Invocation == null) {
				...
				case CLASS:
					...
					aInvocation = aDeduceTypes2._phase().registerClassInvocation(classStatement, null, new ReadySupplier_1<>(aDeduceTypes2)); // TODO generics
//				ClassInvocationMake.withGenericPart(classStatement, null, null, this);
				...
				_p_Invocation =new Eventual<>();_p_Invocation.resolve (aInvocation);
			}
			_p_Invocation.then(cb);
		}
...
//				ClassInvocationMake.withGenericPart(classStatement, null, null, this);
	...
	public static class DeduceTypes2Injector {
		...
		public WlGenerateClass new_WlGenerateClass(final GenerateFunctions aGenerateFunctions, final ClassInvocation aCi, final GeneratedClasses aGeneratedClasses, final ICodeRegistrar aCodeRegistrar) {
			return new WlGenerateClass(aGenerateFunctions, aCi, aGeneratedClasses, aCodeRegistrar);
		}

		public WlGenerateNamespace new_WlGenerateNamespace(final GenerateFunctions aGenerateFunctions, final NamespaceInvocation aCi, final GeneratedClasses aGeneratedClasses, final ICodeRegistrar aCodeRegistrar) {
		...
		public DG_ClassStatement new_DG_ClassStatement(final ClassStatement aClassStatement, final DeduceTypes2 aDeduceTypes2) {
			return new DG_ClassStatement(aClassStatement, aDeduceTypes2);
		}
		...
		public Resolve_Ident_IA new_Resolve_Ident_IA(final DeduceClient3 aDeduceClient3, final Context aContext, final Resolve_Ident_IA.DeduceElementIdent aDei, final BaseEvaFunction aGeneratedFunction, final FoundElement aFoundElement, final ErrSink aErrSink) {
		...
		public ITE_Resolver_Result new_ITE_Resolver_Result(final OS_Element aE) {
			return new ITE_Resolver_Result(aE);
		}
	...
	class df_helper_Constructors implements df_helper<EvaConstructor> {
		...
		public boolean deduce(@NotNull EvaConstructor aEvaConstructor) {
			return deduceOneConstructor(aEvaConstructor, _phase());
		}
	}
	...
	class df_helper_Functions implements df_helper<EvaFunction> {
		...
		public boolean deduce(@NotNull EvaFunction aGeneratedFunction) {
			return deduceOneFunction(aGeneratedFunction, _phase());
		}
	}
	...
	public class DeduceClient4 {
		...
		public ClassInvocation genCI(final @NotNull GenType aType, final TypeName aGenericTypeName) {
			return aType.genCI(aGenericTypeName, deduceTypes2, deduceTypes2._errSink(), deduceTypes2._phase());
		}
		...
		public @NotNull FunctionInvocation newFunctionInvocation(final FunctionDef aElement, final ProcTableEntry aPte, final @NotNull IInvocation aInvocation) {
			return deduceTypes2.newFunctionInvocation(aElement, aPte, aInvocation, deduceTypes2._phase());
		}
		...
		public @NotNull ClassInvocation registerClassInvocation(final @NotNull ClassInvocation aCi) {
			return deduceTypes2._phase().registerClassInvocation(aCi);
		}
		...
		public @Nullable ClassInvocation registerClassInvocation(final @NotNull ClassStatement aClassStatement, final String constructorName) {
			return deduceTypes2._phase().registerClassInvocation(aClassStatement, constructorName, new ReadySupplier_1<>(deduceTypes2));
		}

		public NamespaceInvocation registerNamespaceInvocation(final NamespaceStatement aNamespaceStatement) {
			return deduceTypes2._phase().registerNamespaceInvocation(aNamespaceStatement);
		}
...
///////			LOG.info(String.format("Expectation (%s, %d) set: %s %s", DeduceTypes2.this, counter, desc, base.expectationString()));
	...
	class Resolve_each_typename {
		...
		public void action_USER_CLASS(@NotNull final TypeTableEntry typeTableEntry, @NotNull final OS_Type aAttached) {
			...
			assert c != null;
			_phase().onClass(c, cc -> typeTableEntry.resolve(cc));
		}
	}
	...
	class WlDeduceFunction implements WorkJob {
		...
		public void run(final WorkManager aWorkManager) {
			...
			if (workJob instanceof final WlGenerateFunction workJob1) {
				workJob1.getResultPromise().then(new
														 DoneCallback<BaseEvaFunction>() {
															 @Override
															 public void onDone(final BaseEvaFunction generatedFunction1) {
																 if (!coll.contains(generatedFunction1)) {
																	 ...
																	 if (!generatedFunction1.deducedAlready) {
																		 ...
																		 deduce_generated_function((EvaFunction) generatedFunction1, mt);
																	 }
				...
				((WlGenerateDefaultCtor) workJob).getResultPromise().then(new DoneCallback<BaseEvaFunction>() {
					@Override
			...
			} else if (workJob instanceof final WlGenerateCtor gen) {
				gen.getResultPromise().then(new DoneCallback<EvaConstructor>() {
					@Override
					public void onDone(final EvaConstructor evaConstructor) {
						if (!coll.contains(evaConstructor)) {
							coll.add(evaConstructor);
							if (!evaConstructor.deducedAlready) {
								deduce_generated_constructor(evaConstructor);
...
//				LOG.info("7003 "+vteName+" "+ctx);
				...
				if (best2 != null) {
					...
					_errSink().reportError("Special Function not found " + pn);
				} else {
		...
		private boolean action_i2_IntegerIA_vteName_is_not_null(@NotNull String pn, @Nullable String pn2, @NotNull VariableTableEntry vte) {
			...
			case BUILT_IN -> {
				...
				try {
					...
					_errSink().reportDiagnostic(resolveError);
					return false;
				}
			}
			default -> {
	...
	class Dependencies {
		...
		public void action_function(@NotNull FunctionInvocation aDependentFunction) {
			...
			if (function == LangGlobals.defaultVirtualCtor) {
				...
				gen = _inj().new_WlGenerateDefaultCtor(gf, aDependentFunction, creationContext(), _phase().codeRegistrar);
			} else {
				mod = function.getContext().module();
				final @NotNull GenerateFunctions gf = getGenerateFunctions(mod);
				gen = _inj().new_WlGenerateFunction(gf, aDependentFunction, _phase().codeRegistrar);
			}
			wl.addJob(gen);
		...
		public void action_type(@NotNull GenType genType) {
			...
			if (genType.getResolvedn() != null) {
				...
				final @NotNull GenerateFunctions gf  = _phase().generatePhase.getGenerateFunctions(mod);
				NamespaceInvocation              ni  = _phase().registerNamespaceInvocation(genType.getResolvedn());
				@NotNull WlGenerateNamespace     gen = _inj().new_WlGenerateNamespace(gf, ni, _phase().generatedClasses, _phase().codeRegistrar);
				...
				final Eventual<ClassDefinition> pcd = _phase().generateClass(gf, ci);
