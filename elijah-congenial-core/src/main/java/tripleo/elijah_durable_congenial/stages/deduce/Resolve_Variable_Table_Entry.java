...
import tripleo.elijah_durable_congenial.lang.impl.FuncTypeNameImpl;
...
import tripleo.elijah_durable_congenial.stages.deduce.tastic.DT_External_2;
...
//			tripleo.elijah.util.Stupidity.println_out_2("1630 "+mod_ns.getItems()); // element 0 is ctor$0
			...
			@NotNull GenerateFunctions     gen  = _phase().generatePhase.getGenerateFunctions(mod1);
			...
			assert pte != null;
			callable_pte = pte;
			@NotNull FunctionInvocation fi = _phase().newFunctionInvocation(fd1, pte, modi);
			wl.addJob(deduceTypes2._inj().new_WlGenerateNamespace(gen, modi, _phase().generatedClasses, _phase().codeRegistrar)); // TODO hope this works (for more than one)
			final @Nullable WlGenerateFunction wlgf = deduceTypes2._inj().new_WlGenerateFunction(gen, fi, _phase().codeRegistrar);
			wl.addJob(wlgf);
			wm.addJobs(wl);
			wm.drain(); // TODO here?
	...
	private void genCIForGenType(final @NotNull GenType aGenType) {
		...
		aGenType.genCI(aGenType.getNonGenericTypeName(), deduceTypes2, deduceTypes2._errSink(), deduceTypes2.getPhase());
